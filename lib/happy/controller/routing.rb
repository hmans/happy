      ...
      def path(*args, &blk)
        ...
        args.each do |name|
          ...
          if name.present?
            ...
            path_match = path_to_regexp(name).match(request.remaining_path.first)
          end

          # Match the request method, if specified
          method_matched = [nil, request.request_method.downcase.to_sym].include?(options[:method])

          path_matched   = (path_match || (name.nil? && request.remaining_path.empty?))
          ...
          if path_matched && method_matched
            ...
            if path_match
              ...
              request.previous_path << request.remaining_path.shift
            end
